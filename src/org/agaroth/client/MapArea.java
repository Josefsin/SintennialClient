...
public class MapArea extends GameFrame {
	...
	enum OrbType {
		...
		HITPOINTS(new int[] { 329, -1, 330 }, new int[] { 1, 2 }), PRAYER(new int[] {
				331, 332, 333 }, new int[] { -2, -2 }), RUN(new int[] { 337,
						338, 335, 336 }, new int[] { 0, -1 }), SUMMONING(new int[] {
								367, 369, 368 }, new int[] { 0, 0 });
	...
	public void displayMoneyPouch(Client client) {
		...
		value = "" + Client.formatAmount(amount);
		if (amount <= 999999) {
			color = 0xffffff;
		} else {
			color = 65280;
	...
	protected void render(Client client, ScreenMode screenMode) {
		if (isVisible()) {
			...
			if (client.inSprite(false, Client.cacheSprite[456], getxPos() + (screenMode != ScreenMode.FIXED ? 130 : 6), getyPos() + (screenMode != ScreenMode.FIXED ? 132 : 124))) {

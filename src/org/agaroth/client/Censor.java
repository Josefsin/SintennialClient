...
public final class Censor {
	...
	public static String doCensor(String s) {
		...
		for (String exception : exceptions) {
			for (int n = -1; (n = s2.indexOf(exception, n + 1)) != -1;) {
				char[] ac1 = exception.toCharArray();
				System.arraycopy(ac1, 0, ac, n, ac1.length);
			}
		}
	...
	public static void loadConfig(Archive archive) {
		ByteBuffer buffer1 = new ByteBuffer(archive.get("fragmentsenc.txt"));
		ByteBuffer buffer2 = new ByteBuffer(archive.get("badenc.txt"));
		ByteBuffer buffer3 = new ByteBuffer(archive.get("domainenc.txt"));
		ByteBuffer buffer4 = new ByteBuffer(archive.get("tldlist.txt"));
		readValues(buffer1, buffer2, buffer3, buffer4);
	}

	private static void method493(ByteBuffer buffer, char ac[][], byte abyte0[][][]) {
		for (int j = 0; j < ac.length; j++) {
			...
			ac[j] = ac1;
			byte abyte1[][] = new byte[buffer.getUnsignedByte()][2];

			for (int l = 0; l < abyte1.length; l++) {
				abyte1[l][0] = (byte) buffer.getUnsignedByte();
				abyte1[l][1] = (byte) buffer.getUnsignedByte();
			}
	...
	private static void method494(char ac[][], ByteBuffer buffer) {
		for (int j = 0; j < ac.length; j++) {
			char ac1[] = new char[buffer.getUnsignedByte()];
	...
	private static void readBadEnc(ByteBuffer stream) {
		int j = stream.getIntLittleEndian();
	...
	private static void readDomainEnc(ByteBuffer stream) {
		int i = stream.getIntLittleEndian();
		aCharArrayArray623 = new char[i][];
		method494(aCharArrayArray623, stream);
	}

	private static void readFragmentsEnc(ByteBuffer stream) {
		anIntArray620 = new int[stream.getIntLittleEndian()];
		for (int i = 0; i < anIntArray620.length; i++) {
			anIntArray620[i] = stream.getUnsignedShort();
		}
	}

	private static void readTldList(ByteBuffer stream) {
		int i = stream.getIntLittleEndian();
	...
	private static void readValues(ByteBuffer stream, ByteBuffer stream_1, ByteBuffer stream_2, ByteBuffer stream_3) {
		readBadEnc(stream_1);
